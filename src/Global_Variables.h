 ...
 *
 * *******************************************************************/
extern volatile uint8_t Measurement_Sequence;   //which state the port is in. Triggered, captured, or Reset
extern uint8_t Input_Pin;
extern volatile long Period;           //time between start of each sample
extern volatile uint8_t Sequence_Position;      //if the ports are triggered in sequence, which port is 1st, 2nd, etc
extern volatile uint8_t Detected_Port_Count;   //how many active ports triggered their interrupt
extern volatile bool Detected;                  //has given port triggered its interrupt
extern volatile long On_Time;          //current on time for he given port
